...
#Sobre: Script baseado em https://github.com/zx-ecco/Skunkworks/blob/master/xubuntu-14.04-desktop-install-fest.sh alterado por Luis e Pedro para suportar outras variantes do Ubuntu.

#2-do - Adicionar e testar funcionalidade no Debian e variantes.
#OUTRAS NOTAS:
#-O script foi testado rodando sem erros nas versões x86 do Mint, Xubuntu e Ubuntu.
...
#receita para sistema com apps decentes/suporte crypto a partir de install (sua letra favorita aqui)ubuntu 14.04.
...
sudo apt-get update
sudo apt-get install --yes linux-image-generic linux-headers-generic linux-firmware linux-tools-generic
#app purge commands are now segmented so that if one app is gi allready uninstalled the script will still remove the rest.
...
#Lubuntu - Necessita de gcc e make
if [ $VERSAUM == "LUBUNTU"]; then
sudo apt-get --yes install gcc make 
fi
